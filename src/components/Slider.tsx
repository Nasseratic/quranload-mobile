// THIS FILE COPIED FROM https://github.com/flyskywhy/react-native-smooth-slider/tree/master
...
export default class Slider extends PureComponent {
  ...
  _onHandlerStateChange = (event) => {
    if (event.nativeEvent.state === State.BEGAN) {
      ...
      // This synthetic event is reused for performance reasons, so save it first
      const translationX = this.props.vertical
      ...
      (event.nativeEvent.state === State.FAILED || event.nativeEvent.state === State.END)
    ) {
  ...
  setValue = (value: number) => {
    if (!this.isMoving && this.state.containerSize.width) {
      this._translateX.setOffset(
        this.props.vertical
          ? this.state.containerSize.width - this._getThumbCenter(value)
          : this._getThumbCenter(value)
      );
  ...
  render() {
    ...
    const thumbCenter = value.interpolate({
    ...
    let translate = this._translateX.interpolate({
    ...
    return (
      ...
      >
        ...
        >
          {trackImage ? (
            <Image style={trackStyle} source={trackImage} resizeMode="stretch" />
          ) : (
            <Animated.View
              renderToHardwareTextureAndroid={true}
        ...
        <Animated.View
          ...
          style={[
            ...
            {
              transform: [{ translateX: translate }, { translateY: 0 }],
              ...valueVisibleStyle,
            },
          ]}
        >
  ...
  _handlePanResponderGrant = (x: number) => {
    ...
    this._translateX.setOffset(
      this.props.vertical ? this.state.containerSize.width - this._lastOffsetX : this._lastOffsetX
    );
    this._translateX.setValue(0);
  ...
  _handlePanResponderMove = (e: any) => {
    ...
    const velocityX = e.nativeEvent.velocityX;

    const velocityY = e.nativeEvent.velocityY;
    ...
    const translation = this.props.vertical ? -translationY : translationX;
    ...
    const newValue = this._getValue(offset);
    if (this._getCurrentValue() !== newValue) {
      this._setCurrentValue(newValue);
      const velocity = Math.abs(this.props.vertical ? velocityY : velocityX);
  ...
  _handlePanResponderEnd = (translationX: number) => {
    ...
    const value = this._getValue(this._lastOffsetX);
    const offset = this._getThumbCenter(value);
    this._translateX.setOffset(
      this.props.vertical ? this.state.containerSize.width - offset : offset
    );
    this._translateX.setValue(0);
  ...
  _handleMeasure = (name: string, x: Object) => {
    ...
    if (this._containerSize && this._trackSize && this._thumbSize) {
      this.setState(
        ...
        () => {
          const offset = this._getThumbCenter(this.props.value);
          this._translateX.setOffset(
            this.props.vertical ? this.state.containerSize.width - offset : offset
          );
          this._translateX.setValue(0);
