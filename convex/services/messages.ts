...
// Utility function for filtering nullish values
function isNotNullish<T>(value: T): value is NonNullable<T> {
  return value !== null && value !== undefined;
}

// Helper for matching conversation types
async function matchConversationType(
  ctx: QueryCtx | MutationCtx,
  conversation:
    | { type: "team"; teamId: string }
    | { type: "direct"; participantX: string; participantY: string }
    | { type: "support"; userId: string }
): Promise<string> {
  if (conversation.type === "support") {
    return `support_${conversation.userId}`;
  }
  if (conversation.type === "team") {
    return conversation.teamId;
  }
  // direct
  return (await getConversationId(ctx, conversation)) ?? "_";
}
...
export const paginate = query({
  ...
  handler: async (ctx, { paginationOpts, conversation }) => {
    const conversationId = await matchConversationType(ctx, conversation);
...
export const send = mutation({
  ...
  handler: async (ctx, { senderId, messages, to }) => {
    ...
    if (to.type === "support") {
      conversationId = `support_${to.userId}`;
    } else if (to.type === "team") {
      conversationId = to.teamId;
    } else {
      conversationId = await getOrCreateDirectConversation(ctx, {
        senderId,
